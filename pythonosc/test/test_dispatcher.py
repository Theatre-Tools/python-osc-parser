...
class TestDispatcher(unittest.TestCase):
    ...
    def test_simple_map_and_match(self):
        ...
        self.sortAndAssertSequenceEqual([Handler(handler, [1, 2, 3])], self.dispatcher.handlers_for_address("/test"))
        self.sortAndAssertSequenceEqual([Handler(handler, [])], self.dispatcher.handlers_for_address("/test2"))
    ...
    def test_example_from_spec(self):
        ...
        for index, address in enumerate(addresses):
            self.sortAndAssertSequenceEqual([Handler(index, [])], self.dispatcher.handlers_for_address(address))
    ...
    def test_match_middle_star(self):
        ...
        self.sortAndAssertSequenceEqual([Handler(2, [])], self.dispatcher.handlers_for_address("/foo/*/2"))
    ...
    def test_match_address_contains_plus_as_character(self):
        ...
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/foo*/bar+*/*"))
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/foo*/bar*/*"))

    def test_call_correct_dispatcher_on_star(self):
        self.dispatcher.map("/a+b", 1)
        self.dispatcher.map("/aaab", 2)
        self.sortAndAssertSequenceEqual([Handler(2, [])], self.dispatcher.handlers_for_address("/aaab"))
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/a+b"))

    def test_map_star(self):
        self.dispatcher.map("/starbase/*", 1)
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/starbase/bar"))

    def test_map_root_star(self):
        self.dispatcher.map("/*", 1)
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/anything/matches"))

    def test_map_double_stars(self):
        self.dispatcher.map("/foo/*/bar/*", 1)
        self.sortAndAssertSequenceEqual([Handler(1, [])], self.dispatcher.handlers_for_address("/foo/wild/bar/wild"))
        self.sortAndAssertSequenceEqual([], self.dispatcher.handlers_for_address("/foo/wild/nomatch/wild"))
